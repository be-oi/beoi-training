...
\tikzset{
    ...
    a/.style={circle},
    b/.style={rectangle, minimum size=.65cm},
...
\begin{center}
\scalebox{0.9}{
\begin{tikzpicture}[a/.style={circle, inner sep=0, minimum size=0.75cm}]
\node[b] at (0,0) (00) {3};
\node[b] at (1,0) (01) {-1};
\node[b] at (2,0) (02) {1};
\node[b] at (3,0) (03) {-4};
...
\only<2->{\draw[choice] (10) to (00);}
\node[a] at (4,1.5) (11) {\only<2->{4}};
\only<2->{\draw[choice] (11) to (05);}
\node[a] at (7,1.5) (12) {\only<2->{0}};
\only<2->{\draw[choice] (12) to (06);}
\node[a] at (10,1.5) (13) {\only<2->{3}};
\only<2->{\draw[choice] (13) to (09);}

\node[b] at (2.5,3) (20) {\only<3->{3}};
\only<3->{\draw[choice] (20) to (10);}
\node[b] at (8.5,3) (21) {\only<3->{0}};
...
\begin{lstlisting}
int minimax(state u) {
    if (isEnding(u)) // the game has ended
        return score(u);
    int best = -INF;
