...
No node in the sub-tree of $v$ has a link to {\color{red} one ancestor of $u$ other than $(u, v)$}.
...
A non-root node $u$ is an articulation point if and only if when we finish $v$, $low[v] \geq num[u]$
